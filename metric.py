# https://github.com/lartpang/Py-SOD-VOS-EvalToolkit/blob/master/metrics/sod/saliency_metric.py
import numpy as np
...
class cal_fm(object):
    ...
    def update(self, pred, gt):
        if gt.max() != 0:
            precision, recall, fm_tmp = self.cal(pred, gt)
            self.precision[self.idx,:] = precision
            self.recall[self.idx,:] = recall
            self.meanF[self.idx,:] = fm_tmp
        self.idx += 1
